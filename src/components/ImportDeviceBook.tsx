...
export function ImportDeviceBook() {
  ...
  return (
    <section className="mb-6 rounded-2xl bg-rose-mist/50 px-4 py-3 ring-1 ring-line">
      <p className="text-sm font-medium">Import this device’s book</p>
      <p className="mt-1 text-sm text-cocoa-soft">
        This browser still has {legacyCount} client
        {legacyCount === 1 ? '' : 's'} from the old on-device book, and the
        salon in the cloud is empty.
      </p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        >
          {busy ? 'Importing…' : 'Import this device’s book'}
        </button>
        <button
          type="button"
